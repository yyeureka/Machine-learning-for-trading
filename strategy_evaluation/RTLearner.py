...
class RTLearner(object):
    ...
    def add_evidence(self, data_x, data_y):
        ...
        if self.verbose:
            print("Random tree shape: ", self.learner.shape)
            print("Random tree details:")
            print(self.learner)
    ...
    def build_tree(self, data_x, data_y, height):
        ...
        # Get random feature i and split value
        best_i = np.random.randint(0, data_x.shape[1])
        split_val = (data_x[np.random.randint(0, data_x.shape[0]), best_i]
                     + data_x[np.random.randint(0, data_x.shape[0]), best_i]) / 2
